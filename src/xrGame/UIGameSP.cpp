...
bool CUIGameSP::IR_UIOnKeyboardPress(int dik)
{
    ...
    case kSCORES:
    {
        if (!pActor->inventory_disabled())
        {
            m_game_objective = AddCustomStatic("secondary_task", true);
            ...
            if (t1 && t2)
            {
                m_game_objective->m_static->SetTextST(t2->m_Title.c_str());
            }
            else
            {
                if (t1 || t2)
                {
                    ...
                    m_game_objective->m_static->TextItemControl()->SetTextST(t->m_Description.c_str());
                }
                else
                {
